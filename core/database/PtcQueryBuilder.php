...
class PtcQueryBuilder
{
	...
	public function select($columns)
	{
		...
		foreach ($columns as $v) {
			...
			if (preg_match('/\(([^\)]*)\)/', $column[0], $matches)) {
				...
				$col = $this->addBackTicks($column[0]);
			}
	...
	{
		if ($val = $this->_checkRawValue($string)) {
			return $val;
		}
		$raw = explode('.', $string);
	...
	{
		if (!$this->_checkOperator($operator)) {
			return;
		}
	...
	public function find($id)
	{
		if (!$this->_isTableSet()) {
			return;
		}	// check if table property is set
		$this->_where = null;
	 ...
	 * @return	object single column value if "$column" argument is set, the full row otherwise
	 */
	public function first($column = null): object
	{
		if (!$this->_isTableSet()) {
			return false;
		}
	...
	protected function _buildQuery()
	{
		switch ($this->_currentQueryType) {
			...
			case 'update':
				...
				break;
			case 'delete':
				$query = 'DELETE FROM ' . $this->_table . $this->_where;
				break;
			case 'select':
			default:
	...
	protected function _executeSql($mode = 1)
	{
		...
		if (!empty($this->_fetchMode)) {
			if (array_key_exists(1, $this->_fetchMode)) {
				...
				$this->_query->setFetchMode($this->_fetchMode[0]);
			}
			$this->_fetchMode = array();
		}
		switch ($mode) {
			case 3:
				$result = $this->countRows();
				break;
			case 2:
				 ...
				 */
				$result = array_filter((array) $result, function ($key) {
	...
	protected function _buildBetweenClause($column, $start, $end, $type = 'between')
	{
		...
		$this->_where .= $type . ' ';
		if ($val = $this->_checkRawValue($start)) {
			$this->_where .= ' ' . $val;
		} else {
			$this->_where .= ' ? ';
		...
		$this->_where .= ' AND ';
		if ($val = $this->_checkRawValue($end)) {
			$this->_where .= ' ' . $val;
		} else {
			$this->_where .= ' ? ';
	...
	protected function _fireEvent($data)
	{
		...
		if (@$listeners['query']) {
			$event_class::fire('ptc.query', $data);
		}
	}
	/**
	...
	protected static function _debug($string, $statement = null, $category = 'QueryBuilder')
	{
		...
		return @call_user_func_array(
			array(_PTCDEBUG_NAMESPACE_, 'bufferSql'),
